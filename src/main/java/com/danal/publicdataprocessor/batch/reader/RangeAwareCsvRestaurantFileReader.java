...
/**
 * RangeAwareCsvRestaurantFileReader
 *
 * startLine, endLine 범위를 지정해 해당 범위만 처리하는 Reader
 */
public class RangeAwareCsvRestaurantFileReader extends FlatFileItemReader<RestaurantCsvRowData> {

    private final Set<String> processedManagementNumbers = new HashSet<>();
    private final int startLine;
    private final int endLine;
    private int currentLine = 1; // 헤더를 스킵한 상태로 초기화

    public RangeAwareCsvRestaurantFileReader(String filePath, FieldSetMapper<RestaurantCsvRowData> fieldSetMapper,
    ...
    public RestaurantCsvRowData read() throws Exception {
        ...
        while ((item = super.read()) != null) {
            ...
            if (currentLine < startLine) {
                continue; // startLine 이전 데이터는 스킵
            }

            if (currentLine > endLine) {
                return null; // endLine 이후 데이터는 중단
            }
